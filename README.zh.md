...
悬停到一个已经带图标的链接上时，按钮会变成 **重新格式化**（重新查询一次，替换标题与图标），旁边还多出一个 **重置** 按钮。重置会把图标去掉，只留下最普通的 Markdown 链接：

```
[Example Domain](https://example.com)
```

重置完全依据笔记里已有的文本就地完成：不发请求、不用等待，链接地址也原样保留。
...
- 「重置」只回到 `[标题](链接)`，而不是这一行第一次被格式化之前的样子：裸链接一旦格式化，原文就已被网站标题取代，再往回退只能靠猜。
