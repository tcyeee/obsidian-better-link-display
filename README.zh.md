...
一个 Obsidian 插件：编辑笔记时把裸链接一键"格式化"成带网站名称与图标的书签。

## 功能说明

在编辑模式下把鼠标悬停到一个裸链接上，链接上方会浮出一个 **格式化** 按钮：
...
- 悬浮按钮只在编辑模式（实时预览或源码模式）下出现，且只针对独立成立的裸链接——代码块、frontmatter 以及已有的 Markdown 链接内部不会触发。
