...
Only the pure-string modules (`src/urlScan.ts`, `src/verbatim.ts`, `src/bookmarkScan.ts`, `src/tableScan.ts`) have tests — they import nothing from `obsidian`, which is what makes them runnable outside the app. Keep new text/parsing logic there rather than inline in the editor extension so it stays testable.
...
**Links inside a table** never reach step 1, because Live Preview replaces the whole table with one rendered *block* widget and CodeMirror's `hoverTooltip` gives up on those: `BlockWidgetView.coordsAt()` reports a rect flattened onto the widget's left edge, and `startHover()` refuses to open a tooltip for a pointer more than one character width from the position it found — so for a link in the middle of a cell the hover source is never so much as called. That menu is therefore shown by hand, through a `Tooltip` held in `tableMenuField` and provided to `showTooltip`, opened and closed by `mousemove`/`mouseleave` listeners the plugin attaches to `view.dom` itself. Not through `EditorView.domEventHandlers`: those pass through `eventBelongsToEditor()`, which drops events inside a widget whose `ignoreEvent()` says so — the default, and exactly the widget this exists for. `view.dom` rather than the content, because the menu is drawn inside the former, so leaving the editor is heard while moving onto the menu is not.

`src/tableScan.ts` maps the rendered link back to source: `td.cellIndex` and `tr.rowIndex` give the cell, and since the delimiter row is not rendered, a body row sits one line lower in the source than in the table. That arithmetic assumes the two grids line up, so it is never trusted alone — the cell it lands on has to contain the hovered link's own URL, and `findExternalLinkAt()` has to agree there is a link there; anything else offers no menu rather than a menu that would rewrite the wrong cell. Three consequences worth keeping: the hover source refuses to run at all while the pointer is inside a table (near the left edge it *is* called, with the table's start position, which the text path would happily scan for some other row's link); the menu is the tooltip element itself rather than sitting inside a hover host, so `styles.css` has a second rule putting back the box CodeMirror paints on it; and the loading and failure highlights are decorations on text the widget hides, so a format started from a table shows nothing until the table re-renders.
